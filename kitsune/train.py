...
def train(
    ...
    dp = build_input_data_pipe(
        str(input_path), batch_size=batch_size, shuffle=True, file_format=file_format
    )
    ...
    if n_samples % batch_size != 0:
        raise Exception(
            f" n_samples {n_samples} modulo batch_size {batch_size} should be zero but is not, choose a different batch size"
        )

    if pretrained_mapper is None:
        logging.info("🦊 Training feature mapper ...")

        feature_mapper = build_feature_mapper(
            dp,
    ...
    for epoch in range(1, epochs + 1):
        model, losses = train_single_epoch(
            model=model,
            ds=dp,
            optimizer=optimizer,
            device=device,
